...
get_commands_for_language() {
    ...
    case "$lang" in
        *"Python"*)
            echo "cd backend && pytest && ruff check ."
            ;;
        *"Rust"*)
...
get_language_conventions() {
    ...
    case "$lang" in
        *"Python"*)
            echo "Python: 遵循仓库约定（ruff + mypy + pytest；backend/）"
            ;;
        *)
...
update_existing_agent_file() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        if [[ "$line" == "## Active Technologies" ]]; then
            ...
            # Drop placeholder entry from older runs
            continue
        ...
        if [[ "$line" == "## Recent Changes" ]]; then
            ...
            # Drop extra blank lines immediately after new entry
            continue
            ...
            # Drop placeholder entries from older runs
            if [[ "$line" == *"[if applicable,"* ]]; then
                continue
            fi
            if [[ "$line" == "- $CURRENT_BRANCH: Added" ]]; then
                continue
            fi
